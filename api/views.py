...
class RegisterUserView(generics.GenericAPIView):
    ...
    def post(self, request,registerInForm):
        print(request.data)
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        user = registerInForm.save()
        # _, token = AuthToken.objects.create(user)
        login(self.request, user)  # if you readily want to login the user after register
        redirect("index.html")
        return Response({
            "user" : UserSerializer(storeRegView, context=self.get_serializer_context()).data,
            # "token":token
        })
